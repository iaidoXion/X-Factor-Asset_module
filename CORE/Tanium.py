...
def minutely_plug_in():                                                                     # 변수 명 Full Name : Full Name에서 대문자로 명시한 것들을 뽑아서 사용 (괄호 안의 내용은 설명)
    ...
    DSB = SETTING['PROJECT']['DASHBOARD'].lower()
    VUL = SETTING['PROJECT']['VUL'].lower()
    # projectType
    ProjectType = []
    if DSB == 'true' :
        ProjectType.append('DSB')
    if VUL == 'true' :
        ProjectType.append('VUL')
    # SOURCE
    for project in ProjectType :
        if SOMIPAPIU == 'true':     # (Source Data MINUTELY Input plug in API 사용 여부 확인 - 사용함.)
            ...
            SDIPDL = CIATSCPI(SK, project)['dataList']                             # Source Data InPut Data List (Tanium API Sensor Data 호출)
        # input plug in 이 API 외의 것들 구현 예정
        
        SODDFT = CTDAAPI(SDIPDL, 'API', project)                                                         # Source Data Data Frame Transform (호출한 데이터를 Data Frame 형태로 변형)
