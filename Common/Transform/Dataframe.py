...
def plug_in(data, dataType, inputPlugin) :
    if dataType == 'asset' :
        DFC = [
        ...
        DFC = ['today_computer_id', 'today_last_reboot', 'today_disk_used_space', 'yesterday_disk_used_space',
               'today_ipv_address', 'today_listen_port_count', 'yesterday_listen_port_count',
               'today_established_port_count', 'yesterday_established_port_count',
               'today_ram_use_size', 'today_ram_total_size', 'today_installed_applications_name',
               'today_running_processes', 'today_cup_consumption']
    DFL = []
    for d in data :
        if dataType == 'asset':
            if inputPlugin == 'API' :
            ...
            TRP = d[12]
            TCC = d[13]
            DFL.append([CID, LR, TDUS, YDUS, TIP, TLPC, YLPC, TEPC, YEPC, TRUS, TRTS, TIAN, TRP, TCC])
